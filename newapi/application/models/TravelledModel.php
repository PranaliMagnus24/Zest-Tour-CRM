...
class TravelledModel extends CI_model
{
    public function get_Travelled_data()
    {
        ...
        $this->db->select('*,i.no_of_pax as nof,i.remark as i_remark,e.enquiry_id as enquiry_number,c.id as customer_id,c.name as customer_name');
        $this->db->from('(SELECT enquiry_id, MIN(id) as id, MIN(no_of_pax) as no_of_pax, MIN(remark) as remark, MIN(booking_date) as booking_date, MIN(product) as product, MIN(supplier_name) as supplier_name, MIN(booking_reference) as booking_reference, MIN(pax_name) as pax_name, MIN(destination) as destination, MIN(cancellation) as cancellation, MIN(checkin) AS mcheckin, MAX(checkout) AS mcheckout, SUM(product_amount) as amount FROM itinerary WHERE is_deleted = 0 GROUP BY enquiry_id) as i,enquiry as e,customer as c');
        $this->db->where('e.id=i.enquiry_id');  
        $this->db->where('e.customer_id=c.id');  
        $this->db->where($where); 
...
<?php
     ...
     public function get_search_Travelled_data()
    {
        ...
        $this->db->select('*,i.no_of_pax as nof,i.remark as i_remark,e.enquiry_id as enquiry_number,c.id as customer_id,c.name as customer_name');
        $this->db->from('(SELECT enquiry_id, MIN(id) as id, MIN(no_of_pax) as no_of_pax, MIN(remark) as remark, MIN(booking_date) as booking_date, MIN(product) as product, MIN(supplier_name) as supplier_name, MIN(booking_reference) as booking_reference, MIN(pax_name) as pax_name, MIN(cancellation) as cancellation, MIN(checkin) AS mcheckin, MAX(checkout) AS mcheckout, SUM(product_amount) as amount FROM itinerary WHERE is_deleted = 0 GROUP BY enquiry_id) as i,enquiry as e,customer as c');
        $this->db->where('e.id=i.enquiry_id');  
        $this->db->where('e.customer_id=c.id');  
        $this->db->where($where); 
        if(trim($param['type_customer'])!='')
        ...
        $this->db->from('(SELECT enquiry_id, MIN(id) as id, MIN(remark) as remark, MIN(booking_date) as booking_date, MIN(product) as product, MIN(supplier_name) as supplier_name, MIN(booking_reference) as booking_reference, MIN(pax_name) as pax_name, MIN(cancellation) as cancellation, MIN(checkin) AS mcheckin, MAX(checkout) AS mcheckout, SUM(product_amount) as amount, SUM(no_of_pax) as nof FROM itinerary WHERE is_deleted = 0 GROUP BY enquiry_id) as i,enquiry as e,customer as c');
