...
Class UserModel extends CI_model
{
    ...
    public function login()
    {
        ...
        $array = array("email" => $param['email'], "password" => $param["password"], "type"=>$param['type']);
        $this->db->where($array);
        $q=$this->db->get('user');
        $data=$q->result_array();
        if (count($data) > 0) 
    ...
    public function addUser()
    {
        ...
        if ($param['user_id'] == '') {
            ...
            $data=array("success"=>true,"message"=>"User registered successfully","user_id"=>$this->db->insert_id());   
            echo json_encode($data);
        }
        else{
