...
class ConfirmModel extends CI_model
{
    public function get_Confirm_data()
    {
        ...
        if (!isset($param['column']) || trim($param['column']) == '') {
            $param['column'] = 'i.cancellation';
            $param['sort'] = 'desc';
        }
        $this->db->select('*,i.no_of_pax as nof,i.remark as i_remark,e.enquiry_id as enquiry_number');
        $this->db->from('(SELECT enquiry_id, MIN(checkin) AS mcheckin, MAX(checkout) AS mcheckout, SUM(product_amount) as amount, MAX(no_of_pax) as no_of_pax, MAX(remark) as remark, MAX(product) as product, MAX(cancellation) as cancellation, MAX(booking_date) as booking_date, MAX(supplier_name) as supplier_name, MAX(booking_reference) as booking_reference, MAX(pax_name) as pax_name FROM itinerary GROUP BY enquiry_id) as i,enquiry as e');
...
<?php
       ...
       public function get_search_Confirm_data()
     {
        ...
        // $this->db->from('(SELECT *, MIN(checkin) AS mcheckin, MAX(checkout) AS mcheckout,SUM(product_amount) as amount FROM itinerary  WHERE is_deleted = 0  GROUP BY enquiry_id) as i,enquiry as e');  
        // $this->db->where("i.mcheckout>='".date('Y-m-d', strtotime('-7 days'))."'");
...
$this->db->select('
    ...
    SUM(i.no_of_pax) as nof,
    MAX(i.remark) as i_remark,
    MAX(i.cancellation) as cancellation,
    MAX(i.booking_date) as booking_date,
    MAX(i.product) as product,
    MAX(i.supplier_name) as supplier_name,
    MAX(i.booking_reference) as booking_reference,
    MAX(i.pax_name) as pax_name,
    e.enquiry_id as enquiry_number
');
