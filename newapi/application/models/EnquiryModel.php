    ...
    public function get_countries()
{
    ...
    public function getMissingReminder()
    {
        ...
        $this->db->select('r.customer_id, r.time as maxtime, r.reminder');
        $this->db->from('reminder r');
        $this->db->join('(SELECT customer_id, MAX(time) AS maxtime FROM reminder GROUP BY customer_id) as sub', 'r.customer_id = sub.customer_id AND r.time = sub.maxtime', 'inner');
...
<?php
    ...
    public function get_search_Enquiry_data()
    {
        ...
        $this->db->select('sub.customer_id,sub.time,reminder');
        $this->db->from('reminder');
        $this->db->join('(SELECT customer_id, MAX(time) AS time FROM reminder GROUP BY customer_id) as sub', 'reminder.customer_id = sub.customer_id AND reminder.time = sub.time', 'inner');
        $this->db->where('sub.time >=',date('Y-m-d'));
        $this->db->order_by('sub.time', "ASC");
        $q=$this->db->get();
        $reminder_array=$q->result_array();
        ...
        $this->db->select('*')
            ->from('enquiry as e')
            ->join('customer as c', 'e.customer_id = c.id', 'left')
            ->where($where);
        if(trim($param['type_customer'])!='')
        {
    ...
    ->where($where);
        ...
        //echo $this->db->last_query();
        $sorted_data_first=$q->result_array();
        // echo "Display count: " . count($sorted_data_first);
        $counts = count($sorted_data_first);
...
<div class="col-12 p-0 pull-left border-top b-l-r Flex"
    ...
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis abc position-relative" style="width: 10%;"
        onmouseover="show_tooltip(event,'<?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['name'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['name'] : '' ?></div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,'<?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['corporate'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['corporate'] : '' ?></div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,'<?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['number'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['number'] : '' ?></div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,'<?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['email'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['email'] : '' ?></div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,'<?php echo isset($user_data[$value['assigned_id']]) ? $user_data[$value['assigned_id']]['name'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($user_data[$value['assigned_id']]) ? $user_data[$value['assigned_id']]['name'] : '' ?></div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 6%;"
        onmouseover="show_tooltip(event,'<?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['reference'] : '' ?>')"
        onmouseout="hide_tooltip(event)"><?php echo isset($customer_data[$value['customer_id']]) ? $customer_data[$value['customer_id']]['reference'] : '' ?>
    </div>
...
public function get_search_Missing_Reminder()
{
    ...
    $this->load->database();
    $param = array_merge($this->input->get(), $this->input->post());
    if (!isset($param['missed'])) {
        $param['missed'] = 'yes';
    }
    error_log('get_search_Missing_Reminder param: ' . json_encode($param));
    $this->db->select('count(*) as total_reminders');
    $total_q = $this->db->get('reminder');
    $total = $total_q->row()->total_reminders;
    error_log('Total reminders in DB: ' . $total);
    $where = array();
    if(trim($param['user_type'])=='domestic' || trim($param['user_type'])=='international')
    {
        $where['e.type'] = $param['user_type'];
    ...
    $this->db->select('r.reminderby, r.customer_id, r.time, r.reminder');
    $this->db->from('reminder r');
    $this->db->join('(SELECT customer_id, MAX(time) AS maxtime FROM reminder GROUP BY customer_id) as sub', 'r.customer_id = sub.customer_id AND r.time = sub.maxtime', 'inner');
    if($param['select_staff_id']!='' && $param['select_staff_id']!="null")
    {
        $this->db->where('r.reminderby',$param['select_staff_id']);
    }
    ...
    $q=$this->db->get();
    error_log('get_search_Missing_Reminder query: ' . $this->db->last_query());
    $reminder_array=$q->result_array();
    error_log('get_search_Missing_Reminder reminder_array count: ' . count($reminder_array));
        ...
        if(count($reminder)>0)
        {
            ...
            foreach($sorted_data_first as $key=>$value)
            {
                $start_date=date('Y-m-d',strtotime($reminder_customer[$value['customer_id']]['time']));
                $last3_date=date('Y-m-d', strtotime('+4 days'));
                $output .= '<div class="col-12 p-0 pull-left border-top b-l-r Flex"
    ondblclick="redirect(' . $value['customer_id'] . ', \'customer_detail\')"
    style="background-color: #ebeefe;display: flex;">
    <div class="pull-left Flex-item font-blue text-ellipsis" style="width: 4%;padding-left: 8px;">' . $i++ . '
    </div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis position-relative" style="width: 12%;"
        onmouseover="show_tooltip(event,\'' . $value['enquiry_id'] . '\')" onmouseout="hide_tooltip(event)">
        ' . $value['enquiry_id'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis abc position-relative" style="width: 10%;"
        onmouseover="show_tooltip(event,\'' . $customer_data[$value['customer_id']]['name'] . '\')"
        onmouseout="hide_tooltip(event)">' . $customer_data[$value['customer_id']]['name'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . $customer_data[$value['customer_id']]['corporate'] . '\')"
        onmouseout="hide_tooltip(event)">' . $customer_data[$value['customer_id']]['corporate'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . $customer_data[$value['customer_id']]['number'] . '\')"
        onmouseout="hide_tooltip(event)">' . $customer_data[$value['customer_id']]['number'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . $customer_data[$value['customer_id']]['email'] . '\')"
        onmouseout="hide_tooltip(event)">' . $customer_data[$value['customer_id']]['email'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . $user_data[$value['assigned_id']]['name'] . '\')"
        onmouseout="hide_tooltip(event)">' . $user_data[$value['assigned_id']]['name'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . $customer_data[$value['customer_id']]['reference'] . '\')"
        onmouseout="hide_tooltip(event)">' . $customer_data[$value['customer_id']]['reference'] . '</div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 6%;">' . $value['destination'] . '
    </div>
    <div class="col-1 pull-left Flex-item font-blue text-ellipsis" style="width: 6%;">' . $value['days'] . '</div>
    <div class="col-2 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        onmouseover="show_tooltip(event,\'' . 'Adult-'.$value['adult'].'<br>Child-'.$value['children'].'<br>Infant-'.$value['infant'].'<br>Total-'.($value['adult']+$value['children']+$value['infant']) . '\')"
        onmouseout="hide_tooltip(event)">
        ' . 'A-'.$value['adult'].', C-'.$value['children'].', I-'.$value['infant'] . '</div>
    <div class="col-2 pull-left Flex-item font-blue text-ellipsis p-0 text-center" style="width: 8%;">
        ' . ($value['start_date'] == '0000-00-00' ? '' : date('d M Y', strtotime($value['start_date']))) . '
    </div>
    <div class="col-2 pull-left Flex-item font-blue text-ellipsis" style="width: 7%;"
        ' . (array_key_exists($value['customer_id'], $remark) ? 'onmouseover="show_tooltip(event,\'' . $remark[$value['customer_id']]['remark'] . '\')" onmouseout="hide_tooltip(event)"' : '') . '>
        ' . (array_key_exists($value['customer_id'], $remark) ? $remark[$value['customer_id']]['remark'] : '') . '
    </div>
    ';
                if($start_date<$today){$tool_data='No Reminder';}elseif($start_date==$today){$tool_data='Todays';}elseif($start_date>$today && $start_date<=$last3_date){$tool_data='Recent 3 days';}else{$tool_data='After 3 days';}
                $fill_color = '';
                if($param['missed']=='yes') {
                    $fill_color = "black";
                } elseif ($start_date < $today) {
                ...
                $stroke = ($start_date < $today) ? 'stroke: black;stroke-width: 50px;' : '';
                $output .= '<div class="col-2 position-relative pull-left Flex-item font-blue text-center text-ellipsis"
        onmouseover="show_tooltip(event,\'' . $tool_data . '\')" onmouseout="hide_tooltip(event)"
        style="width: 5.5%;padding-top: 4px;"><svg xmlns="http://www.w3.org/2000/svg" class="ionicon"
            viewBox="0 0 512 512" fill="' . $fill_color . '" width="20" style="' . $stroke . '">
            <path d="M256 464c-114.69 0-208-93.31-208-208S141.31 48 256 48s208 93.31 208 208-93.31 208-208 208z" />
        </svg></div>
</div>';
            }
            $output .= ',,,' . $total_count;
            $this->load->model('Pageination');
            $second_count = $total_count;
            $output .= ',,,' . $this->Pageination->get_pageination($total_count, $limit, 2, $param['page'], $param['func'], $second_count);
        }
        else
        {
            $output = ',,,$0';
            $this->load->model('Pageination');
            $output .= ',,,' . $this->Pageination->get_pageination(0, $limit, 2, $param['page'], $param['func'], '');
        }
        return $output;
    }
